...
// PCSX2X6_NETPLAY_JVS_SYNC
static bool s_netplay_jvs_active = false;
static ModernNetplay::ArcadeJvsBundle s_netplay_jvs_bundle{};

static u16 NetplayJvsButtons(u32 player)
{
	if (s_netplay_jvs_active && player < s_netplay_jvs_bundle.size())
		return s_netplay_jvs_bundle[player].buttons;
	u16 value = m_jvsButtonState[player] | m_jvsMacroButtonState[player];
	if (player == 0 && s_lightgun_link_2p)
		value |= m_gunMapping->link_2p;
	return value;
}

static u16 NetplayJvsCoin(u32 player)
{
	return (s_netplay_jvs_active && player < s_netplay_jvs_bundle.size()) ?
		s_netplay_jvs_bundle[player].coin : ACJV::coin[player];
}

static u16 NetplayJvsDrumChannel(u32 channel)
{
	if (!s_netplay_jvs_active)
		return (channel < JVS_DRUM_CHANNEL_MAX) ? m_jvsDrumChannels[channel] : 0;

	switch (channel)
	{
		case 0: return s_netplay_jvs_bundle[0].drum[0];
		case 3: return s_netplay_jvs_bundle[0].drum[1];
		case 5: return s_netplay_jvs_bundle[0].drum[2];
		case 4: return s_netplay_jvs_bundle[0].drum[3];
		case 2: return s_netplay_jvs_bundle[1].drum[0];
		case 7: return s_netplay_jvs_bundle[1].drum[1];
		case 1: return s_netplay_jvs_bundle[1].drum[2];
		case 6: return s_netplay_jvs_bundle[1].drum[3];
		default: return 0;
	}
}

static void SyncNetplayJvsState()
{
	s_netplay_jvs_active = false;
	if (!ModernNetplay::IsConfigured())
		return;

	const ModernNetplay::StatusSnapshot status = ModernNetplay::GetStatusSnapshot();
	if (!status.start_committed || status.local_player_id < 1 || status.local_player_id > 2)
		return;
	...
	const u32 player = status.local_player_id - 1;
	ModernNetplay::ArcadeJvsState local{};
	local.mode = static_cast<u32>(m_jvsMode);
	local.buttons = m_jvsButtonState[player] | m_jvsMacroButtonState[player];
	if (player == 0 && s_lightgun_link_2p)
		local.buttons |= m_gunMapping->link_2p;
	local.coin = ACJV::coin[player];
	local.screen_x = m_jvsScreenPosX[player];
	local.screen_y = m_jvsScreenPosY[player];
	local.raw_x = s_gunRawX[player];
	local.raw_y = s_gunRawY[player];

	static constexpr u32 drum_map[2][4] = {{0, 3, 5, 4}, {2, 7, 1, 6}};
	for (u32 i = 0; i < 4; i++)
		local.drum[i] = m_jvsDrumChannels[drum_map[player][i]];
	for (u32 i = 0; i < JVS_WHEEL_CHANNEL_MAX; i++)
		local.analog[i] = m_jvsWheelChannels[i];

	local.dip_switch_state = s_dip_switch_state;
	local.test_state = m_testButtonState;
	local.flags = s_lightgun_link_2p ? 1u : 0u;
	s_netplay_jvs_active = ModernNetplay::SynchronizeArcadeJvs(local, &s_netplay_jvs_bundle);
}
void do_jvs_packet(const u8* input, u8* output) {
	...
	while(inSize != 0) {
		...
		case JVS::READ_INP_SWITCH:
		{
			...
			(*output++) = JVS_CMD_SUCCESS;
			const u16 netplay_test = s_netplay_jvs_active ?
				(s_netplay_jvs_bundle[0].test_state | (s_netplay_jvs_bundle[0].dip_switch_state & TESTMODE)) :
				(m_testButtonState | (s_dip_switch_state & TESTMODE));
			(*output++) = static_cast<u8>(netplay_test);
			//(*output++) = (m_jvsSystemButtonState == 0x03) ? 0x80 : 0;  //Test

			const u16 p1btn = NetplayJvsButtons(0);
			...
			if(playerCount == 2)
			{
				const u16 p2btn = NetplayJvsButtons(1);
		...
		case JVS::READ_INP_COIN:
		{
			...
			(*output++) = static_cast<u8>(((NetplayJvsCoin(0) >> 8) & 0x3f) | (slot1Condition << 6)); //Coin 1 MSB + slot1condition
			(*output++) = static_cast<u8>(NetplayJvsCoin(0) & 0x00ff);                                //Coin 1 LSB

			(*dstSize) += 3;

			if(slotCount == 2)
			{
				(*output++) = static_cast<u8>(((NetplayJvsCoin(1) >> 8) & 0x3f) | (slot2Condition << 6)); //Coin 2 MSB + slot2condition
				(*output++) = static_cast<u8>(NetplayJvsCoin(1) & 0x00ff);                                //Coin 2 LSB
		...
		case JVS::READ_INP_ANALOG:
		{
			...
			if(m_jvsMode == JVS_MODE::LIGHTGUN)
			{
				...
				if (!s_netplay_jvs_active)
					UpdateLightgunFromMouse();
				const u16 net_x = s_netplay_jvs_active ? s_netplay_jvs_bundle[0].screen_x : m_jvsScreenPosX[0];
				const u16 net_y = s_netplay_jvs_active ? s_netplay_jvs_bundle[0].screen_y : m_jvsScreenPosY[0];
				(*output++) = static_cast<u8>(net_x >> 8); //Pos X MSB
				(*output++) = static_cast<u8>(net_x);      //Pos X LSB
				(*output++) = static_cast<u8>(net_y >> 8); //Pos Y MSB
				(*output++) = static_cast<u8>(net_y);      //Pos Y LSB
			}
			else if(m_jvsMode == JVS_MODE::DRUM)
			{
				...
				{
					(*output++) = static_cast<u8>(NetplayJvsDrumChannel(i) >> 8);
					(*output++) = static_cast<u8>(NetplayJvsDrumChannel(i));
				}
			}
			else if(m_jvsMode == JVS_MODE::DRIVE)
			{
				...
				{
					u16 v = (i < JVS_WHEEL_CHANNEL_MAX) ?
						(s_netplay_jvs_active ? s_netplay_jvs_bundle[0].analog[i] : m_jvsWheelChannels[i]) : 0x8000;
		...
		case JVS::READ_INP_SCREENPOS:
		{
			...
			if (s_netplay_jvs_active)
			{
				const auto& state = s_netplay_jvs_bundle[pl];
				if (m_jvsMode == JVS_MODE::LIGHTGUN &&
					(board == GunBoardModel::CameraVN || board == GunBoardModel::TwoTierTC3))
				{
					posX = state.raw_x;
					posY = state.raw_y;
				}
