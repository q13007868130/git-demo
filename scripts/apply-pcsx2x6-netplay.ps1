...
if ($text -notmatch 'PCSX2_MODERN_NETPLAY_MENU') {
    # pcsx2x6 has an older/different MainWindow::setupAdditionalUi() layout and
    # does not call setupStatusBarWidgets(). Insert after its stable display-action setup.
    $needle = "`tupdateDisplayRelatedActions(false, false, false);"
    if (-not $text.Contains($needle)) { throw 'pcsx2x6 MainWindow.cpp setupAdditionalUi anchor not found' }
    $menuCode = @'
...
Write-Host 'PCSX2X6 Modern Netplay base patch applied.' -ForegroundColor Green
